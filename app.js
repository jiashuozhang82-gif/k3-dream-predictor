...
function trainFromSamples(){const done=getSamples().filter(s=>s.result); if(!done.length)return null; let marketHit=0,poissonHit=0; for(const s of done){const marketPick=Object.entries(s.odds).filter(([k])=>["h","d","a"].includes(k)).sort((a,b)=>a[1]-b[1])[0]?.[0]; if(marketPick===s.result)marketHit+=1; if(s.modelPick===s.result)poissonHit+=1;} const marketRate=marketHit/done.length, poissonRate=poissonHit/done.length; const market=Math.round(20+marketRate*45); const poisson=Math.round(20+poissonRate*45); const team=Math.max(10,100-market-poisson); return {count:done.length,marketRate,poissonRate,weights:{market,poisson,team}};}
...
function toggleMonitor(){const btn=$("#toggleMonitor"); if(monitorTimer){clearInterval(monitorTimer); monitorTimer=null; if(btn)btn.textContent="开始监控"; return;} recordCurrentOdds(); const seconds=Math.max(5,+$("#monitorSeconds")?.value||30); monitorTimer=setInterval(()=>{recordCurrentOdds(true);},seconds*1000); if(btn)btn.textContent="停止监控";}
...
function renderAdvancedAnalytics(match, home, away, md, choices){
  ...
  const trained=trainFromSamples(); const threshold=+$("#moveThreshold")?.value||0.08; const isAlert=last && Math.max(Math.abs(odds.h-last.odds.h||0),Math.abs(odds.d-last.odds.d||0),Math.abs(odds.a-last.odds.a||0))>=threshold; $("#dataStats").innerHTML = `总样本 ${samples.length} 条；本场 ${matchSamples.length} 条。${move}${trained?`<br>训练样本 ${trained.count} 条；盘口命中 ${(trained.marketRate*100).toFixed(1)}%，泊松命中 ${(trained.poissonRate*100).toFixed(1)}%。建议权重：盘口 ${trained.weights.market} / 泊松 ${trained.weights.poisson} / 阵容 ${trained.weights.team}`:""}${isAlert?`<div class="monitor-alert">赔率变化超过阈值 ${threshold.toFixed(2)}，建议复核盘口。</div>`:""}`;
...
function recordCurrentOdds(silent=false){const match=currentMatch(); const md=model(match); const choices=[{key:"h",value:md.h},{key:"d",value:md.d},{key:"a",value:md.a}].sort((a,b)=>b.value-a.value); const samples=getSamples(); const current={time:Date.now(),matchId:match.id,odds:readOdds(),modelPick:choices[0].key,handicap:{line:+$("#handicapLine").value,home:+$("#handicapHome").value,away:+$("#handicapAway").value}}; samples.push(current); setSamples(samples); if(!silent)renderMain(); else renderMain();}
